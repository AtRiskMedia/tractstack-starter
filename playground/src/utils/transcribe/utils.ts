...
export const getWordSelectionType = (wordIdx: number, s: WordSelection): SelectedWordType => {
  if (s.startRange >= 0 && s.endRange >= 0) {
    if (wordIdx === s.startRange) return SelectedWordType.FIRST_WORD;
    else if (wordIdx === s.endRange) return SelectedWordType.LAST_WORD;
    else if (wordIdx >= s.startRange && wordIdx <= s.endRange) return SelectedWordType.MIDDLE;
  }
...
export const mergeSelections = (selections: WordSelection[]): WordSelection[] => {
  ...
  const fullQuotes = mergeSelection(
    selections.filter((x) => x.type === TextSelectionType.KEY_POINT)
  );
...
export const firstAndSecondWordAreDifferent = (): boolean => {
  return (
    $firstWordSelected.get().wordIdx !== $secondWordSelected.get().wordIdx &&
