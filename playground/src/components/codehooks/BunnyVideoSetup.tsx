...
const BunnyVideoSetup = ({ nodeId, params, config }: BunnyVideoSetupProps) => {
  ...
  // Construct paneList from paneIds
  ...
  // Construct StoryFragment entry for contentMap
  ...
  // Initialize state from params on first render
  useEffect(() => {
  ...
  // Save changes with optional custom chapters
  const saveChanges = (customChapters?: Chapter[] | React.FocusEvent<HTMLInputElement>) => {
    // If this is called from an onBlur event, ignore the event parameter
  ...
  // Handle pane selection for a chapter - fixed to avoid race condition
  const handlePaneSelect = (index: number, slug: string) => {
    ...
    // Save changes immediately with the updated chapters
    saveChanges(updatedChapters);
  };

  // Get the slug for a linked pane
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {formErrors[`endTime-${index}`] && (
                      <p className="mt-1 text-xs text-red-600">{formErrors[`endTime-${index}`]}</p>
                    )}
                  </div>
                </div>
