...
export default function OrphanFilesList() {
  ...
  const fetchFiles = async () => {
    try {
      ...
      const response = await fetch("/api/turso/getOrphanFiles");
      if (!response.ok) {
  ...
  if ($orphanItems.items.length === 0) {
    return <div className="text-center py-8">No files found.</div>;
  }
    ...
    <div className="mb-8">
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-4 bg-gray-50 rounded">
          No {showAll ? "" : "orphaned "}files found.
        </div>
      ) : (
        <>
          {/* Desktop view - Table */}
