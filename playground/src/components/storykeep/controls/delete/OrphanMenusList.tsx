...
export default function OrphanMenusList() {
  ...
  const fetchMenus = async () => {
    try {
      ...
      const response = await fetch("/api/turso/getOrphanMenus");
      if (!response.ok) {
  ...
  if ($orphanItems.items.length === 0) {
    return <div className="text-center py-8">No menus found.</div>;
  }
    ...
    <div className="mb-8">
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-4 bg-gray-50 rounded">
          No {showAll ? "" : "orphaned "}menus found.
        </div>
      ) : (
        <>
          {/* Desktop view - Table */}
