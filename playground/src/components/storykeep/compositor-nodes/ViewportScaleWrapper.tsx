...
export const ViewportScaleWrapper = ({ 
  children, 
  className = "" 
}: ViewportScaleWrapperProps) => {
  ...
  const calculateScale = (availableWidth: number, targetViewport: string): number => {
    ...
    return SCALE_OPTIONS.find(scale => 
      (minWidth * (scale / 100)) <= availableWidth
    ) || 25; // Default to 25% if nothing else fits
  };
  ...
  useEffect(() => {
    ...
    const newScale = calculateScale(
      containerWidth,
      $viewportKey.value || $viewport.value
    );
  ...
  return (
    <div className={`viewport-scale-container relative w-full overflow-x-auto min-h-screen ${className}`}>
      {children}
    </div>
  );
};
