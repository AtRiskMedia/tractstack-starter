...
import {
  ...
  isFirstWordClicked,
  isSecondWordClicked,
  removeWordSelectionAt,
  setAllWordSelection,
  setWordSelection,
  type WordSelection,
} from "@/store/transcribe/appState.ts";
import {
  firstAndSecondWordAreDifferent,
  getWordSelectionType,
...
const handleWordEdit = (
  props: ChapterWordsEditorProps,
  wordIdx: number,
  changed: (newWord: string) => void
) => {
...
export const ChapterWordsEditor = memo((props: ChapterWordsEditorProps) => {
  ...
  const getSelectMode = (word: Word, wordIdx: number): SelectedWordType => {
    const getSelection = (s: WordSelection): SelectedWordType => {
      if (s.type !== props.textSelectType) return SelectedWordType.NOT_SELECTED;
    ...
    if (activeSelection) {
      ...
      if (selectionType !== SelectedWordType.NOT_SELECTED) return selectionType;
    }
  ...
  const onDeleteSentence = (wordIdx: number) => {
    ...
    const idx = chapterSelections.findIndex(
      (x) => x.type === props.textSelectType && x.endRange === wordIdx
    );
    if (idx !== -1) {
