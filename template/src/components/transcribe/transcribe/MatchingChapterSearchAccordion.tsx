            ...
            <div className="border-b w-full border-slate-200">
                <button onClick={toggleAccordion}
                    ...
                    <div className={`flex ${getColor()} p-2 w-full`}>
                        ...
                        <div className="flex flex-col items-start">
                            ...
                            <p>Gist: <span className="font-bold">{gist}</span></p>
                            <label>
                                Excluded: {excluded}
                                <input type="checkbox"
                                       checked={excluded}
                ...
                <div ref={contentRef} className="max-h-0 overflow-hidden transition-all duration-300 ease-in-out">
                    <div className="pb-5 text-sm text-slate-500">
                        {open &&
                            <ChapterWordsEditor chapter={props.chapter}
                                                textSelectOp={TextSelectionOperation.NONE}
                                                textSelectType={TextSelectionType.NONE}
                                                searchWord={props.searchWord}
                                                active={open}
                                                searchWordLocations={createTimeSearchSet(props.chapter.wordsMap.get(props.searchWord))}/>
                        }
                    </div>
                </div>
            </div>
