...
export const StoryEditor = memo((props: StoryEditorProps) => {
    ...
    const run = async () => {
      try {
        ...
        if (uuid) {
          ...
          const storyUrl =
            `/api/transcribe/stories?` + new URLSearchParams({ uuid: storyuuid }).toString();
          ...
          const convertedData = buildTranscript(
            transcriptResponse.transcript,
            transcriptResponse.sentences,
            transcriptResponse.paragraphs
          );
  ...
  const onChapterEdit = (chapter: Chapter) => {
    ...
    if (storyChapters) gist = storyChapters?.get(chapter.index)?.gist;
    ...
    <div>
      {transcript ? (
        <div className="flex flex-col">
          <a
            className="px-4 py-2 w-fit rounded-md text-lg shadow-sm transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-myorange bg-myorange text-white hover:bg-myblue"
            href={"/transcribe"}
          >
            Back
          </a>
