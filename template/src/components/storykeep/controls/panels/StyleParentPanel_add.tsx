...
const StyleParentPanelAdd = ({ node, layer }: BasePanelProps) => {
  ...
  // Ensure node is a PaneFragmentNode before checking if it's a MarkdownPaneFragmentNode
        ...
        <Combobox value={selectedStyle} onChange={handleSelect}>
          {({ open }) => (
            <>
              <div className="relative">
                <Combobox.Input
                  ...
                  displayValue={(key: string) =>
                    styles.find((style) => style.key === key)?.title || ""
                  }
              ...
              <Combobox.Options
                className={`absolute z-50 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none ${
