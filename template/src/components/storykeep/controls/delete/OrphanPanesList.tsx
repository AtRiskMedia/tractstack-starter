...
export default function OrphanPanesList() {
  ...
  useEffect(() => {
    // Fetch panes when the component mounts
    fetchPanes();
  ...
  const fetchPanes = async () => {
    try {
      ...
      const response = await fetch("/api/turso/getOrphanPanes");
      if (!response.ok) {
        throw new Error("Failed to fetch panes");
      }
      ...
      if (result.success) {
        ...
        throw new Error(result.error || "Failed to fetch panes");
      }
    } catch (error) {
      orphanItemsStore.set({
        items: [],
  ...
  if ($orphanItems.isLoading) {
    return <div className="text-center py-8">Loading panes...</div>;
  }
  ...
  if ($orphanItems.items.length === 0) {
    return <div className="text-center py-8">No panes found.</div>;
  }
  ...
  return (
    <div className="mb-8">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-4">
        <h3 className="text-xl font-semibold">Panes</h3>
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-4 bg-gray-50 rounded">
          No {showAll ? "" : "orphaned "}panes found.
        </div>
      ) : (
        <>
          {/* Desktop view - Table */}
              ...
              {paginatedItems.map((item) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <h4 className="font-medium text-gray-900">{item.title || "Unnamed"}</h4>
                    {item.usageCount === 0 ? (
                      <input
                        type="checkbox"
                        checked={!!$selectedForDeletion[item.id]}
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-900">{item.slug || "N/A"}</span>
                    </div>
